...
// 1) Make sure you Kodiak_Kapture.Dispose before shutting down the app (to avoid hanging)
// 2) Make sure you Marshal.FreeCoTaskMem items returned from Kodiak_Kapture.SnapShot (to avoid leaking)
...
{
    internal class Kodiak_Kapture : IDisposable
    {
        enum GraphState
        {
            Stopped,
        ...
        public event Kodiak_KaptureEvent StopPlay;
        public delegate void Kodiak_KaptureEvent(Object sender);

        // Play an avi file into a window.  Allow for snapshots.
        // (Control to show video in, Avi file to play
        public Kodiak_Kapture(Control hWin, string FileName)
        {
            try
            {
                int hr;
        ...
        private void SetupGraph(Control hWin, string FileName)
        {
            ...
            try
            {
                ...
                m_mediaPosition = m_graphBuilder as IMediaPosition;
